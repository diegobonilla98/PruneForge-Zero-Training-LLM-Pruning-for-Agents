"""Run inference with a pruned Llama model and print the answer."""

from __future__ import annotations

import torch

from prune_llama import build_prompt, load_and_prune_model, normalize_text, read_text


DEFAULT_RECIPE_PATH = "pruning_recipe.json"
DEFAULT_SYSTEM_PROMPT_PATH = "system_prompt_deep_learning.txt"
DEFAULT_QUESTION = (
    "What is the difference between overfitting and underfitting in deep learning?"
)
DEFAULT_MAX_NEW_TOKENS = 128


def generate_answer(
    recipe_path: str,
    system_prompt_path: str,
    question: str,
    device: str,
    dtype: torch.dtype,
    max_new_tokens: int,
) -> str:
    """Load a pruned model and generate a single answer."""
    model, tokenizer = load_and_prune_model(recipe_path, device, dtype)
    system_prompt = read_text(system_prompt_path)
    prompt = build_prompt(tokenizer, system_prompt, question)

    inputs = tokenizer(prompt, return_tensors="pt").to(device)
    gen_ids = model.generate(
        **inputs,
        max_new_tokens=max_new_tokens,
        do_sample=False,
        top_p=1.0,
        num_beams=1,
        pad_token_id=tokenizer.eos_token_id,
        use_cache=False,
    )
    prompt_len = inputs["input_ids"].shape[1]
    gen_tokens = gen_ids[:, prompt_len:]
    decoded = tokenizer.batch_decode(gen_tokens, skip_special_tokens=True)
    return normalize_text(decoded[0])


def main() -> None:
    """Entrypoint for single-question inference with defaults."""
    device = "cuda" if torch.cuda.is_available() else "cpu"
    dtype = torch.bfloat16 if torch.cuda.is_available() else torch.float32
    answer = generate_answer(
        recipe_path=DEFAULT_RECIPE_PATH,
        system_prompt_path=DEFAULT_SYSTEM_PROMPT_PATH,
        question=DEFAULT_QUESTION,
        device=device,
        dtype=dtype,
        max_new_tokens=DEFAULT_MAX_NEW_TOKENS,
    )
    print(answer)


if __name__ == "__main__":
    main()
